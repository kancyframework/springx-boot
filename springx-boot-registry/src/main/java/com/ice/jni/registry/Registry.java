...
//			System.loadLibrary( "ICE_JNIRegistry" );
            DllLoader.loadFromJar("ICE_JNIRegistry");
    ...
    listKeysCommand(RegistryKey topKey, String keyName) {
        ...
        try {
            Enumeration enumTemp = subKey.keyElements();
            for (int kIdx = 0; enumTemp.hasMoreElements(); ++kIdx) {
                String keyStr = (String) enumTemp.nextElement();
                System.err.println
                        ("Subkey[" + kIdx + "] = '" + keyStr + "'");
            }
        } catch (RegistryException ex) {
    ...
    listValuesCommand(RegistryKey topKey, String keyName) {
        ...
        try {
            Enumeration enumTemp = subKey.valueElements();
            for (int kIdx = 0; enumTemp.hasMoreElements(); ++kIdx) {
                String name = (String) enumTemp.nextElement();
                System.err.println
