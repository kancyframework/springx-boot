...
RegistryKey
	...
	public RegistryKey
	createSubKey( String subkey, int access )
			throws RegistryException
	{
	...
	export( PrintWriter out, boolean descend )
		...
		enumTemp = this.valueElements();

		for ( int idx = 0 ; enumTemp.hasMoreElements() ; ++idx )
			{
			String valueName = (String) enumTemp.nextElement();
		...
		if ( descend )
			{
			enumTemp = this.keyElements();

			for ( int idx = 0 ; enumTemp.hasMoreElements() ; ++idx )
				{
				String keyName = (String) enumTemp.nextElement();
